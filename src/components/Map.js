    ...
    <div className="main-map relative mb-3">
      {squares.map((row) =>
        squares.map((column) => {
          ...
          return (
            <div
              style={{ background: isSnake ? "#ffffff" : "#161616" }}
              key={`${row}_${column}`}
              data-x={column}
              data-y={row}
            >
              {isHead && (
                <div className="bg-lime-300 w-full h-full rounded-full"></div>
              )}
              {isFood && (
                <div className="food bg-red-700 w-full h-full rounded-full"></div>
              )}
            </div>
          );
        })
      ...
      {!isStart && (
        <div className="flex flex-col justify-center items-center w-full h-full bg-indigo-500 opacity-75 absolute">
          ...
          <div
            className="border border-solid rounded-md px-5 py-1 bg-white cursor-pointer font-bold"
